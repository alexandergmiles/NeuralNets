...
public class LinearLayerNetwork {
	...
	protected void findDeltas(ArrayList<Double> errors) {
		//Iterating over the collection and assigning the index of deltas i to errors i
	...
	{
		//Returning the value based off of the calculation
		return (wNeuron * (numInputs+1)) + wWeight;
	}
	...
	private void changeOneWeight(int wNeuron, int wWeight, double theIn, double learnRate, double momentum) {
		//Calculate the index
		int index = weightIndex(wNeuron, wWeight);
		//Calculate the changed weight
		changeInWeights.set(index, (learnRate*deltas.get(wNeuron)*theIn)+(changeInWeights.get(index)*momentum));
		//Assign new weight
		weights.set(index, weights.get(index) + changeInWeights.get(index));
	}
	...
	protected void changeAllWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		double theIn;
		for (int i = 0; i < numNeurons; i++) {                   //Iterating over the neurons
			for(int j = 0; j <  numInputs+1; j++)                 //Iterating over weights in the neuron
			{
				if(j == 0) {                                 //Check for bias weight
					theIn = 1.0;
				}
				else {
					theIn = ins.get( j - 1);
				}
				changeOneWeight(i, j, theIn, learnRate, momentum);    //Check the current weight we're on
			}
		}
	}
	...
	public ArrayList<Double> weightedDeltas() {
		...
		for(int i = 0; i < numInputs; i++)
		{
			...
			{
				error += (deltas.get(j)*weights.get(weightIndex(j, i+1)));
			}
			wtDeltas.add(error);
		}
	...
	public String getWeights() {
		...
		for (int ct=0; ct<weights.size(); ct++)			//Iterate over the weights stored in arraylist
			s = s + String.format("%.5f", weights.get(ct)) + " "; //Adding the formatted string values read from the weights list
	...
	public static void main(String[] args) {
		...
		System.out.println(LN.doLearn(7,  0.3,  0.1));
		System.out.println(LN.doPresent());
		System.out.println("Weights " + LN.getWeights());
